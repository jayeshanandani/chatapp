...
export default class extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    this.setState(this.initState(nextProps, index !== nextProps.index))
  }
  ...
  componentWillUpdate(nextProps, nextState) {
    ...
    if (index !== nextState.index) onIndexChanged(nextState.index)
  }
  ...
  onLayout = (event) => {
    ...
    const { total, index, dir } = this.state
  ...
  autoplay = () => {
    ...
    const { autoplayEnd, index, total } = this.state
  ...
  onScrollEnd = e => {
    ...
    if (!e.nativeEvent.contentOffset) {
      if (dir === 'x') {
        ...
        e.nativeEvent.contentOffset = { y: e.nativeEvent.position * height }
      }
    }

    this.updateIndex(e.nativeEvent.contentOffset, dir, () => {
  ...
  scrollBy = (index, animated = true) => {
    ...
    const state = this.state
    const diff = (loop ? 1 : 0) + index + this.state.index
    let x = 0
    let y = 0
   ...
   */
  renderPagination = () => {
    const { activeDot, activeDotColor, activeDotStyle, dotColor, dotStyle, paginationStyle } = this.props
    const { total, index, dir } = this.state
    // By default, dots only show when `total` >= 2
    if (total <= 1) return null

    let dots = []
    const ActiveDot = activeDot || <View style={[{
    ...
    return (
      <View pointerEvents='none' style={[styles['pagination_' + dir], paginationStyle]}>
        {dots}
      </View>
    )
  }
  ...
  renderNextButton = () => {
    ...
    if (loop ||
      index !== total - 1) {
      button = nextButton || <Text style={styles.buttonText}>›</Text>
    }
  ...
  renderPrevButton = () => {
    const { loop, prevButton } = this.props
    let button = null

    if (loop || this.state.index !== 0) {
      button = prevButton || <Text style={styles.buttonText}>‹</Text>
    }
