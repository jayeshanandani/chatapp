...
import { getAnnouncement } from '@redux/announcement/actions'
...
class ArchiveAnnoucement extends PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            active: false,
    ...
    componentDidMount() {
        const { getAnnouncement, getAnnouncementList } = this.props
        // if (!getAnnouncementList.length) {
        getAnnouncement();
        // }
    ...
    render() {
        const { getAnnouncementList, loader, navigation } = this.props
        const nonArchiveAnnouncementList = getAnnouncementList && getAnnouncementList.filter(item => item.isArchive === true)
        return (
            <View style={[common.flex1, common.bgWhite]}>
                ...
                <FlatList
                    ...
                    data={nonArchiveAnnouncementList}
                    renderItem={this.renderItem}
                    extraData={this.props}
                    keyExtractor={(item, index) => 'announcement_' + index}
                    ListEmptyComponent={this._renderEmptyState('No Archive Founds')}
                />
...
export default connect(mapStateToProps, mapDispatchToProps)(ArchiveAnnoucement)