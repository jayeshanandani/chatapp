...
import { createReminder, getReminder } from '@redux/reminder/actions'
...
const createReminderValidation = Yup.object().shape({
...
class CreateReminders extends Component {
  ...
  componentDidMount() {
    ...
    const { getReminder } = nextProps;
    if (nextProps.success) {
      that.toastRef.current.show('Reminder successfully created', 1000);
      getReminder();
  ...
  onClickSave = (values) => {
    const { createReminderAction } = this.props
  ...
  render() {
    ...
    return (
      <KeyboardAvoidingView behavior={Platform.OS == 'ios' ? enums.PADDING : ''} style={[common.flex1, { backgroundColor: 'white' }]}>
        ...
        />
        <Formik
          initialValues={{ reminderName: '', date: '', time: '', remindTime: '', location: 'Ahmadabad' }}
          validationSchema={createReminderValidation}
          onSubmit={values => {
            Keyboard.dismiss();
              ...
              <View style={common.flex1}>
                ...
                <View>
                  ...
                  <View style={[common.mh20, { marginBottom: 10 }]}>
                    <WSDateTimePicker pickerMode='date' code='&#xf073;' getDate={this.onPassProp} />
                  </View>

                  <View style={[common.mh20, { marginTop: 10 }]}>
                    <WSDateTimePicker pickerMode='time' code='&#xf017;' getDate={this.onPassProp} />
                  </View>
...
const mapStateToProps = (state) => {
  return {
    success: state.reminder && state.reminder.reminderSuccess,
    loginUserId: state.user && state.user.userData && state.user.userData.id,
    loader: state.user && state.reminder.createLoading,
    createdReminder: state.user && state.reminder.createReminder,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    createReminderAction: (value) => dispatch(createReminder(value)),
    getReminder: () => dispatch(getReminder()),
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(CreateReminders)